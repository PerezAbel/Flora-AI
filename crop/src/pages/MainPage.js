...
import { FaRobot, FaPaperPlane, FaImages, FaBroadcastTower, FaVolumeUp, FaUserCircle, FaLeaf } from 'react-icons/fa';
import './MainPage.css';
...
const CropAI = () => {
  ...
  const openAction = (action) => {
    ...
    setChatOpen(false);
    setCameraError('');
    if (action !== 'takeSnapshot') setSnapshotUrl('');
    if (action !== 'addSnapshot') setUploadUrl('');
    if (action !== 'live') setLivePosition({ x: 0, y: 0 });
    if (action !== 'scan') setScanTarget(null);
  };
  ...
  const handleLivePointerDown = (event) => {
    setIsDragging(true);
    dragOriginRef.current = { x: event.clientX, y: event.clientY };
    dragStartRef.current = { ...livePosition };
  ...
  const handleLivePointerUp = (event) => {
    setIsDragging(false);
    event.currentTarget.releasePointerCapture?.(event.pointerId);
  };

  useEffect(() => {
    if (activeAction === 'takeSnapshot' || activeAction === 'live' || activeAction === 'scan') {
      startCamera();
    } else {
      stopCamera();
    }
  ...
  const scanTargets = [
    {
      ...
      summary: 'Early blight risk detected.',
      details: 'Humidity high, lesions on lower leaves. Apply copper-based spray.'
    },
    {
      id: 'plot-b',
      name: 'Plot B · Maize',
      summary: 'Nitrogen deficiency signs.',
      details: 'Pale leaves and slow growth. Side-dress nitrogen fertilizer.'
    },
    {
      id: 'shed-1',
      name: 'Barn · Cattle #12',
      summary: 'Mild dehydration signs.',
      details: 'Low water intake. Increase clean water access and monitor 24h.'
    }
  ];
   ...
   return (
    <div className="ai-assistant-container">  
        ...
        <div className="profile-settings">
          <div className="profile-pill" aria-label="User profile">
            ...
            <div className="profile-info">
              <span className="profile-name">Flora User</span>
              <span className="profile-email">floragarden@gmail.com</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="nav-welcome">
        ...
        <span className="nav-welcome-name">Flora User</span>
      </div>
 
      
      <div className="ai-assistant-content">
        <div className="ai-title-container">
          <div className="ai-title-logo" aria-hidden="true">
            <FaLeaf />
          </div>
          <h1 className="ai-title">Flora AI</h1>
          <p className="ai-subtitle">Lets Make the World a greener place</p>
        </div>
        ...
        <div className="ai-agent-section">
          ...
          <div className="chat-with-agent">
            {activeAction && (
              <div className="action-panel" role="region" aria-live="polite">
                <div className="action-panel-header">
                  <div>
                    <span className="action-panel-kicker">Active mode</span>
                    <h3 className="action-panel-title">
                      {activeAction === 'takeSnapshot' && 'Take snapshot'}
                      {activeAction === 'addSnapshot' && 'Add snapshot'}
                      {activeAction === 'live' && 'Live interaction'}
                      {activeAction === 'scan' && 'Scanning'}
                    </h3>
                  </div>
                  <button
                ...
                {activeAction === 'takeSnapshot' && (
                  <div className="action-panel-body">
                    <div className="camera-stage">
                      {cameraError && <div className="camera-error">{cameraError}</div>}
                      <video ref={videoRef} className="camera-video" muted playsInline />
                      {snapshotUrl && (
                        <img className="camera-preview" src={snapshotUrl} alt="Snapshot preview" />
                      )}
                    </div>
                    <div className="camera-controls">
                      <button className="action-btn primary" type="button" onClick={handleCapture}>
                        Capture
                      </button>
                      <button
                ...
                {activeAction === 'addSnapshot' && (
                  <div className="action-panel-body">
                    <label className="upload-card">
                      <span className="upload-title">Choose a snapshot from your device</span>
                      <span className="upload-subtitle">JPG, PNG, or HEIC supported.</span>
                    ...
                    {uploadUrl && (
                      <div className="upload-preview">
                        <img src={uploadUrl} alt="Uploaded snapshot preview" />
                      </div>
                    )}
                  </div>
                )}
                ...
                {activeAction === 'live' && (
                  <div className="action-panel-body">
                    <div className="live-area">
                      <div
                        className={`live-stage ${isDragging ? 'dragging' : ''}`}
                        style={{ transform: `translate(${livePosition.x}px, ${livePosition.y}px)` }}
                      ...
                      >
                        ...
                        <div className="live-hint">Drag to reposition view while scanning the farm.</div>
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <div className="action-panel-body">
                    <div className="scan-stage">
                      <div className="scan-view">
                        ...
                        <div className="scan-targets" aria-label="Scan targets">
                          {scanTargets.map((target) => (
                            <button
                              key={target.id}
                              type="button"
                              className={`scan-target ${scanTarget?.id === target.id ? 'active' : ''}`}
                              onMouseEnter={() => setScanTarget(target)}
                              onMouseLeave={() => setScanTarget(null)}
                              onFocus={() => setScanTarget(target)}
                              onBlur={() => setScanTarget(null)}
                            >
                              <span className="scan-dot"></span>
                              <span className="scan-label">{target.name}</span>
                            </button>
                          ))}
                        </div>
                      </div>
                      <div className="scan-status">
                        ...
                        <p>Hover over a target in the camera view to scan it.</p>
                        <div className="scan-progress">
                          <span className="scan-progress-bar"></span>
                        </div>
                      </div>
                      <div className={`scan-result ${scanTarget ? 'active' : ''}`}>
                        {scanTarget ? (
                          <>
                            <h4>{scanTarget.name}</h4>
                            <p>{scanTarget.summary}</p>
                            <span>{scanTarget.details}</span>
                          </>
                        ) : (
                          <p>No target selected. Hover over a marker to view results.</p>
                        )}
                      </div>
                    </div>
                  </div>
          ...
          <div className="input-wrapper">
              ...
              {chatOpen && (
                <div className="chat-actions-dropdown" role="menu">
                  <button
                    ...
                    role="menuitem"
                    type="button"
                    onClick={() => openAction('takeSnapshot')}
                  >
                  ...
                  <button
                    ...
                    role="menuitem"
                    type="button"
                    onClick={() => openAction('addSnapshot')}
                  >
                    <FaImages className="action-icon" />
                  ...
                  <button
                    ...
                    role="menuitem"
                    type="button"
                    onClick={() => openAction('live')}
                  >
                    <FaBroadcastTower className="action-icon" />
            ...
            />
            <button className="speaker-btn" type="button" aria-label="Speaker">
              <FaVolumeUp />
            </button>
            <button className="send-btn-new" onClick={handleSendMessage}>
              <FaPaperPlane />
            </button>
          </div>
        </div>  
